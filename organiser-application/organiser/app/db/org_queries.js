...
exports.insertProject = function (project, func) {
    ...
    var tu = [];
    //INSERT INTO `user` (`name`) VALUES ('John');
    var sql = "INSERT INTO project (user_id, title, description, pr_start_date, pr_due_date) VALUES ?";
    tu.push(project.user_id);
    tu.push(project.title);
    tu.push(project.description);
    tu.push(project.pr_start_date);
    tu.push(project.pr_due_date);
...
exports.insertUser = function (user, func) {
    ...
    exports.checkExistingUser(user, function (result) {
        ...
        con.query(sql, [user.firstName, user.lastName, user.username, user.password, user.email], function (err, result) {
            if (err) throw err;
            console.log("Number of records inserted: " + result.affectedRows);
            resultMessage = { status: 200, message: ' Record inserted!' };
            func(resultMessage);
        });
...
//get all projects for the user account
...
exports.checkUserLogin = function (res, user, func) {
